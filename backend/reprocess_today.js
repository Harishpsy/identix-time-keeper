...
// Copy of processDailySummary from attendanceController.js
    ...
    if (existing.length > 0) {
        await pool.execute(
            `UPDATE daily_summaries SET 
             ...
             WHERE id = ?`,
            [firstIn, lastOut, totalDurationMinutes, totalBreakMinutes, lateMinutes, status, firstPunch.ip_address, firstPunch.latitude, firstPunch.longitude, firstPunch.device_name, firstPunch.os_name, firstPunch.browser_name, existing[0].id]
        );
    } else {
        const id = uuidv4();
        await pool.execute(
...
async function run() {
    ...
    console.log(`Reprocessing summaries for ${today}...`);
    ...
    for (const row of users) {
        ...
        console.log(`Processed user: ${row.user_id}`);
    }
    console.log('Done.');
    process.exit(0);
}
